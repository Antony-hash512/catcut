...
Запустить скачивание локальных ИИ-моделей можно из прямо из веб-интерфейса бэкенда, движок `faster-whisper` автоматически скачивает выбранную модель (например, `small`) с репозитория Hugging Face (`Systran/faster-whisper-small`) бэкенд кэширует её в локальный каталог пользователя:
